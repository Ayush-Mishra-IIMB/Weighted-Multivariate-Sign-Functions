...
> **Update:** JMVA weights corrected to use Z = Σ⁻¹/² (Xᵢ − μ) as per JMVA paper Section 2. JMVA p-values computed via permutation test (B_perm = 199).
...
**P-value:** computed via permutation test with $B_\text{perm} = 199$ permutations — no distributional assumption required, valid at any p and n.
...
| Weight | None — $\|Z_i\|=1$ always | $W_i \in (0,1]$ from depth of $\Sigma^{-1/2}(X_i-\hat\mu)$ |
| Location | Origin assumed as centre | Sample mean $\hat\mu$ as centre |
| Null | $N(0,1)$ via martingale CLT | Permutation — exact for any p, n |
...
| JMVA permutations | 199 per test |
| Seed | set.seed(2025) |
...
### Professor's correction — weight standardisation

Following the professor's feedback on the JMVA paper Section 2, the weight functions were corrected. The paper specifies that weights must be computed from the standardised version:

$$Z_i = \Sigma^{-1/2}(X_i - \mu)$$

not from the raw centred observations $(X_i - \mu)$. The $\Sigma^{-1/2}$ is computed via eigendecomposition for numerical stability and computed once per simulation cell for efficiency.

### Remaining issue — JMVA size under Normal

JMVA-PD gives 1.000 on all Normal size rows. The permutation test centres the data at the sample mean before permuting. Under Normal with true mean zero, the sample mean is close to but not exactly zero. At p=1000 and n=20, this small deviation is amplified by the high dimensionality and the permuted statistics do not accurately represent the null distribution of the observed statistic. This is an open problem: deriving a valid asymptotic variance for the weighted sign U-statistic at p >> n remains unsolved. The normal approximation that works for JASA (which relies on ||Zi|| = 1 exactly) does not directly extend to JMVA where ||Ri|| = Wi varies.

### JMVA size under t₃ and Mixture

JMVA-PD size rows under t₃ and Mixture range from 0.41 to 0.99. These are also inflated. The permutation test picks up the heavy-tail structure of the data even under the null because the sample mean used for centring carries information about the distribution. This further motivates developing a proper asymptotic theory for the JMVA U-statistic in the high-dimensional regime.
...
*Simulation: R, MASS package, set.seed(2025), B=300, B_perm=199.*  
*Code file: `final_unified_v5.R`*
